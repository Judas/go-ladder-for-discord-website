...
- **House**: four of them, keyed by `slug` (`FILS_DU_FROID`, `NEXUS_ALPHA`, `SABRE_SILENCIEUX`, `LUNAIRES_AETHER`). Crests follow the shields' convention at `public/crests/{SLUG}.svg` — this repo's call, the server plan leaves it open — and go through `Components/Crest.jsx`. **The four files are placeholders**; swapping in real artwork is a file change, not a code change. `Components/SeasonBanner.jsx` renders the `period`/`season` pair that every house and league response carries; never compute a season or a period on the client, the server ships both for that reason. ⚠ **Never add up the seven scoring columns either.** `points.total` is not their sum: the server divides the scale by the board — 19×19 as is, 13×13 halved, 9×9 quartered, rounded up, other formats not scored at all — so on a small-board player the columns come to more than the points they actually hold. The columns are the detail of how a game was judged, `total` is what it was worth and the only figure anyone is ranked on. `BOARD_SCALING` in `Pages/House.jsx` is the legend that explains this to players, and the only place on the site that does.
- **Joining a house**: `POST /api/house/join` takes `{discordId, slug}` and `POST /api/house/choice` takes `{discordId, action, slug}` — the slug required on `CHANGE`, ignored on `STAY`/`LEAVE`, and a 400 when it names the player's own house. The server **draws no house any more**, so the site is what leads a player to one: a ten-question quiz, four answers each (one per house). ⚠ It **guides, it does not choose** — the end of it is a bilan, the four houses ranked by affinity with a percentage each and a join button under every crest in that house's colour, so the player can join one the answers did not point to. Nothing draws lots any more, ties included: two houses level at the top are two houses offered. ⚠ The questions and the scoring exist **only in `src/houseQuiz.js`**, like the FGC thresholds below — the server takes a slug and asks no questions. The ten questions asked are **drawn from a pool of twenty-two** and their answers shuffled (`drawQuiz`), once per opening — no two players sit the same quiz. Editing the pool touches that file alone, since `Components/HouseQuiz.jsx` only knows the shape and the tests only check the invariants. An answer never reveals which house it scores for — a readable scale turns the quiz into a menu; the bilan at the end names all four, which is where the choice is made. The house's name, colour and motto always come from `/api/houses`; only the slug is the site's.
